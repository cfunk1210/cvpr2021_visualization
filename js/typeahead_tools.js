...
let calcAllKeys = function (allPapers, allKeys) {
    ...
    const collectPrimarySubjectArea = new Set();
    ...
    allPapers.forEach(
      d => {
          ...
          collectPrimarySubjectArea.add(d.content.primary_subject_area);
      });
    allKeys.authors = Array.from(collectAuthors);
    allKeys.keywords = Array.from(collectKeywords);
    allKeys.recs = Array.from(collectRecs);
    allKeys.primary_subject_area = Array.from(collectPrimarySubjectArea);
